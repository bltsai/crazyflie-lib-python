...
def do_action(cf, action):
    ...
    global has_interrupt, key, zrange, xw, range_front
    ...
    if mode == TAKE_OFF or mode == LANDING:
        ...
        if abs(4-xw) > 1:
            print('Start rotate\n')
            print('range_front:{}, xw:{}'.format(range_front, xw))
            cf.commander.send_hover_setpoint(action[0], action[1], YAWRATE, action[3])
        elif abs(4-xw) > 0:
            # yawrate = ((4-xw)/4.0)*30
            if range_front < 1000 and range_front >= 500:
                vx = 0.5
            elif range_front < 500 and range_front >= 0:
                vx = -0.5
            else:
                vx = 0
            print('Start follow\n')
            print('range_front:{}, xw:{}, vx:{}'.format(range_front, xw, vx))
            cf.commander.send_hover_setpoint(vx, action[1], YAWRATE, action[3])
        else:
