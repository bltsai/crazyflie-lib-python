...
# B, G, R
COLOR = {'WHITE':(255, 255, 255), 'RED':(0, 0, 255), 'ORANGE':(0, 128, 255), 'YELLOW':(0, 255, 255), 'GREEN':(0, 255, 0), 'BLUE':(255, 0, 0)}
...
FOLLOW   = 4
PREDICT_ROUTE = 5
MODE = ['TAKE OFF', 'HOVER', 'LANDING', 'INTERRUPT', 'FOLLOW', 'PREDICT_ROUTE']

DESIRED_HEIGHT_CM = 100
TIME_FACTOR   = 3

YAWRATE = 20 # degree/sec
...
def draw_text(frame):
    ...
    cv2.putText(frame,'zMax:'+str(zMax),(x_coordinate, y_coordinate-TEXT_HEIGHT_GAP-3*TEXT_HEIGHT), font, 1,(255,255,255),2,cv2.LINE_AA)
    cv2.putText(frame,'zMin:'+str(zMin),(x_coordinate, y_coordinate-TEXT_HEIGHT_GAP-2*TEXT_HEIGHT), font, 1,(255,255,255),2,cv2.LINE_AA)
    cv2.putText(frame,'range_front:'+str(range_front),(x_coordinate, y_coordinate-TEXT_HEIGHT_GAP-TEXT_HEIGHT), font, 1,(255,255,255),2,cv2.LINE_AA)
    cv2.putText(frame,'mode:'+MODE[mode],(x_coordinate, y_coordinate-TEXT_HEIGHT_GAP), font, 1,(255,255,255),2,cv2.LINE_AA)
...
def trajectory_generator():
    ...
    if mode == TAKE_OFF:
        ...
        if abs(3.5-xw) > 1:
            ...
            if range_front < 1000 and range_front >= 900:
                vx = 0.5
            elif range_front < 900 and range_front >= 800:
                vx = 0.4
            elif range_front < 800 and range_front >= 700:
                vx = 0.3
            elif range_front < 700 and range_front >= 600:
                vx = 0.2
            elif range_front < 600 and range_front >= 500:
                vx = 0.1
            elif range_front < 500 and range_front >= 400:
                vx = -0.1
            elif range_front < 400 and range_front >= 300:
                vx = -0.2
            elif range_front < 300 and range_front >= 200:
                vx = -0.3
            elif range_front < 200 and range_front >= 100:
                vx = -0.4
            elif range_front < 100 and range_front >= 0:
                vx = -0.5
            else:
                vx = 0
            yawrate = YAWRATE / 2
        ...
        if (3.5-xw) > 0:
            ...
            trajectory.append( (vx,0,(-1)*yawrate,DESIRED_HEIGHT_CM/100) )
...
# PARAM_ADD(PARAM_FLOAT, thre_high, &zscore_threshold_high)
    ...
    global mode
    if mode == HOVER:
        cf.param.set_value('zscore.thre_high', '8')
        cf.param.set_value('zscore.thre_low', '7')

        if abs(3.5-xw) < 1 and range_front < 1000:
            change_mode(FOLLOW)

    elif mode == FOLLOW:
        cf.param.set_value('zscore.thre_high', '7')
        cf.param.set_value('zscore.thre_low', '6')
...
def do_action(cf, action):
    ...
    elif mode == HOVER or mode == FOLLOW:
    ...
    def draw():
        while(cap.isOpened()):
            ...
            if ret==True:
                ...
                drawThermal(frame)
                drawHeatCenter(frame, xw, yw,  COLOR['RED'])
                draw_text(frame)
                out.write(frame)
