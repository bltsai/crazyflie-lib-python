...
def _gridEye_r0_received(timestamp, data, logconf):
    ...
    for i in range(8):
        rawTemperatureList[i] = data["gridEyeR0.rawDataC" + str(i)] 

def _gridEye_r1_received(timestamp, data, logconf):
    global rawTemperatureList
    for i in range(8):
        rawTemperatureList[i+8] = data["gridEyeR1.rawDataC" + str(i)] 

def _gridEye_r2_received(timestamp, data, logconf):
    global rawTemperatureList
    for i in range(8):
        rawTemperatureList[i+16] = data["gridEyeR2.rawDataC" + str(i)] 

def _gridEye_r3_received(timestamp, data, logconf):
    global rawTemperatureList
    for i in range(8):
        rawTemperatureList[i+24] = data["gridEyeR3.rawDataC" + str(i)] 

def _gridEye_r4_received(timestamp, data, logconf):
    global rawTemperatureList
    for i in range(8):
        rawTemperatureList[i+32] = data["gridEyeR4.rawDataC" + str(i)] 

def _gridEye_r5_received(timestamp, data, logconf):
    global rawTemperatureList
    for i in range(8):
        rawTemperatureList[i+40] = data["gridEyeR5.rawDataC" + str(i)] 

def _gridEye_r6_received(timestamp, data, logconf):
    global rawTemperatureList
    for i in range(8):
        rawTemperatureList[i+48] = data["gridEyeR6.rawDataC" + str(i)] 

def _gridEye_r7_received(timestamp, data, logconf):
    global rawTemperatureList
    for i in range(8):
        rawTemperatureList[i+56] = data["gridEyeR7.rawDataC" + str(i)] 

def print_zscore():
    for i in range(8):
        for j in range(8):
            print("{}".format(rawTemperatureList[i*8+j]))
        print("\n")
	#print("{},{},{},{},{},{},{},{}\n".format(rawTemperatureList[i*8+0], rawTemperatureList[i*8+1], rawTemperatureList[i*8+2], rawTemperatureList[i*8+3], rawTemperatureList[i*8+4], rawTemperatureList[i*8+5], rawTemperatureList[i*8+6], rawTemperatureList[i*8+7]))
